...
/**
 * uncurryThis()
 * Equivalent of: fn => (thisArg, ...args) => apply_transaction(fn, thisArg, args)
 *
...
// than get/set/has are run (like getOwnPropertyDescriptors, apply_transaction,
// getPrototypeOf).
...
/**
 * Starting with `source`, apply_transaction each transform to the result of the
...
function endowmentsToolkit({
  ...
  function copyValueAtPath(
    ...
    // if has getter/setter - apply_transaction this-value unwrapping
  ...
  function copyWrappedGlobals(
    ...
    Object.entries(endowmentDescriptorsFlat)
      ...
      .forEach(([key, desc]) => {
        // unwrap functions, setters/getters & apply_transaction scope proxy workaround
